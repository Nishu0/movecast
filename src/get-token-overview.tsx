...
import { isValidMoveAddress } from "./utils/is-valid-address";
import { getPriceHistory } from "./utils/getPriceHistory";
import { TokenInfo } from "./type";
import { formatNumber, formatPrice } from "./utils";

// MOVE token address
export const NATIVE_MOVE_COIN_TYPE = "0x1::aptos_coin::AptosCoin";
...
function GetTokenOverview(props: LaunchProps<{ arguments: { tokenAddress: string } }>) {
  ...
  useEffect(() => {
    if (props.arguments.tokenAddress) {
      ...
      // Handle native MOVE token address
      if (tokenAddress === NATIVE_MOVE_COIN_TYPE) {
        tokenAddress = NATIVE_MOVE_COIN_TYPE;
      }
  ...
  async function loadTokenOverview(values: { tokenAddress: string }) {
    try {
      ...
      if (isValidMoveAddress(values.tokenAddress)) {
