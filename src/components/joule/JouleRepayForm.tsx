  ...
  async function handleSubmit(values: {
    ...
    if (!values.tokenType) {
      await showToast({
        ...
        message: "Please select a token to repay",
      });
      return;
    }
    ...
    const confirmMessage = `Repay ${amount} ${token.displayName} to position ${values.positionId}?`;
    ...
    // Execute repay
    await showToast({ style: Toast.Style.Animated, title: `Repaying ${token.displayName}...` });
  ...
  return (
    ...
    >
      ...
      <Form.Dropdown
        id="tokenType"
        title="Select Token"
        info="Choose the token you want to repay"
      ...
      <Form.TextField
        ...
        placeholder="0.0"
        info={selectedToken 
          ? `Amount of ${selectedToken.displayName} to repay`
      ...
      <Form.TextField
        ...
        info="The Joule position ID to repay to"
      />
    </Form>
  );
}
