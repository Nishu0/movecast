...
async function fetchVaaData(): Promise<Uint8Array[]> {
  try {
    ...
    console.log("[Joule Withdraw] Fetching VAA data for price feeds:", priceFeedIds);
    ...
    console.log("[Joule Withdraw] Raw VAA data count:", data.binary.data.length);
    ...
    const vaaData = data.binary.data.map((vaa: string) => {
      ...
      console.log("[Joule Withdraw] VAA bytes length:", bytes.length);
      return new Uint8Array(bytes);
    });

    console.log("[Joule Withdraw] Fetched VAA data, count:", vaaData.length);
    return vaaData;
  } catch (error) {
    console.error("[Joule Withdraw] Error fetching VAA data:", error);
    throw new Error(`Failed to fetch Pyth price data: ${error instanceof Error ? error.message : String(error)}`);
  }
}

/**
 * Withdraw tokens from a Joule position
 ...
 * @param decimals Decimals of the asset being withdrawn (default 8 for MOVE)
...
export async function withdrawToken(
  ...
  try {
    console.log("[Joule Withdraw] Starting withdraw operation...");
    console.log("[Joule Withdraw] Asset type:", assetType);
    console.log("[Joule Withdraw] Amount:", amount);
    console.log("[Joule Withdraw] Position ID:", positionId);
    console.log("[Joule Withdraw] Decimals:", decimals);
    console.log("[Joule Withdraw] Network:", network);
    ...
    console.log("[Joule Withdraw] Position ID (parsed):", positionIdNum);

    console.log("[Joule Withdraw] Creating account from private key...");
    const account = getAccountFromPrivateKey(privateKeyHex);
    console.log("[Joule Withdraw] Account address:", account.accountAddress.toString());

    console.log("[Joule Withdraw] Getting Aptos client...");
    const client = getClient(network);

    // Fetch VAA data from Pyth
    console.log("[Joule Withdraw] Fetching VAA data from Pyth...");
    const vaaData = await fetchVaaData();

    // Convert amount to smallest unit based on token decimals
    const amountInSmallestUnit = BigInt(Math.floor(amount * Math.pow(10, decimals)));
    console.log("[Joule Withdraw] Amount in smallest unit:", amountInSmallestUnit.toString());

    // Build transaction with Joule contract
    // Function signature: withdraw<T0>(position_id: u64, amount: u64, vaa_data: vector<vector<u8>>)
    const functionName = `${JOULE_CONTRACT}::pool::withdraw`;
    console.log("[Joule Withdraw] Building transaction...");
    console.log("[Joule Withdraw] Function:", functionName);
    console.log("[Joule Withdraw] Type arguments:", [assetType]);
    console.log("[Joule Withdraw] Position ID:", positionIdNum);
    console.log("[Joule Withdraw] Amount:", amountInSmallestUnit.toString());
    console.log("[Joule Withdraw] VAA data entries:", vaaData.length);
    ...
    console.log("[Joule Withdraw] Transaction built successfully");

    // Sign and submit the transaction
    console.log("[Joule Withdraw] Signing and submitting transaction...");
    const pendingTransaction = await client.signAndSubmitTransaction({
      signer: account,
      transaction,
    });
    console.log("[Joule Withdraw] Transaction submitted, hash:", pendingTransaction.hash);

    // Wait for transaction to be confirmed
    console.log("[Joule Withdraw] Waiting for transaction confirmation...");
    const committedTransaction = await client.waitForTransaction({
      transactionHash: pendingTransaction.hash,
    });
    console.log("[Joule Withdraw] Transaction confirmed, success:", committedTransaction.success);

    // Check if transaction was successful
    if (!committedTransaction.success) {
      console.error("[Joule Withdraw] Transaction failed:", committedTransaction);
      throw new Error(`Withdraw failed: ${committedTransaction.vm_status || "Unknown error"}`);
    }

    console.log("[Joule Withdraw] Withdraw operation completed successfully!");
    return {
      hash: pendingTransaction.hash,
      positionId,
    };
  } catch (error) {
    console.error("[Joule Withdraw] Error:", error);
